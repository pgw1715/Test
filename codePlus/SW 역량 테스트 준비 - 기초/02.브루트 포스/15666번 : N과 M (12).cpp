...
void makePermutation(vector<vector<int>>&, vector<int>&, int*, int, int, int, int);
...
int main(){
	...
	int permutation[8];
	makePermutation(uniqueList, numList, permutation, n, m, 0, 0);
...
void makePermutation(vector<vector<int>>& uniqueList, vector<int>& numList, int* permutation, int n, int m, int pick, int start){
	if(pick == m){
		vector<int> tmp;
		for(int i=0;i<m;i++){
	...
	for(int i=start;i<n;i++){
		...
		makePermutation(uniqueList, numList, permutation, n, m, pick+1, i);
	}
}
